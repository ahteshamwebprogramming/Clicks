...
{
    [ViewComponent(Name = "Theme")]
    public class ThemeViewComponent : ViewComponent
    {
        private readonly ClientController _clientSettingAPIController;
        private readonly IHttpContextAccessor _httpContextAccessor;
        public ThemeViewComponent(ClientController clientSettingAPIController, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<IViewComponentResult> InvokeAsync()
        {
            ...
            if (int.TryParse(HttpContext.Session.GetString("ClientId"), out output) == true)
            {
                ...
                return View("Theme", clientSettingDTO);
            }
            else
            {
